'''
Implementación del algoritmo RSA para el intercambio de claves con optimizaciones mínimas
para manejar números de 32 bits.
...
def cifrar(M, llave_publica):
    n, e = llave_publica
    return pow(M, e, n)  #Optimizado

def descifrar(C, llave_privada):
    n, d = llave_privada
    return pow(C, d, n)  #Optimizado

def euclides(a, b):
    return math.gcd(a, b)  #Optimizado 
...
def coprimo(On):
    # Números primos comunes 
    for e in [65537, 257, 17, 5, 3]:
        if e < On and math.gcd(e, On) == 1:
            return e
